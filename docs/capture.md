...
`kPrerollPackets` = 3 backfills 6 frames (60 ms) at gate-open, a little under the 62–70 ms bound
rather than over it. The queue itself holds 7 frames (`kHistorySlots` = `kPrerollPackets *
kFramesPerPacket + 1`); the extra slot exists only so the onset frame has somewhere to land on its
way into the burst, not to hold extra backfilled time. The residual is visible in the eval: the
quiet talker measures 10 ms of onset clipping *with* the burst modelled, and would measure zero if
40 ms were the real requirement. That residual is inaudible and well inside the 20–80 ms upstream
Mumble tolerates, so the constant stands — but it covers the blind spot with nothing to spare, not
with margin. This argument is unchanged by the frame-native measurement above: `transmitTrace`
back-fills the same 6 preroll frames into the model's own prediction, so the opening edge cancels by
construction. Onset adequacy is what `MeetsThePinnedBars` scores against labelled speech; this
metric doesn't touch it either way.
