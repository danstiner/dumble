...
void CaptureEngine::onPcm(const int16_t* pcm, uint32_t n) {
    ...
    // Leak guarantee: gate stops ring writes, reset policy clears held frames on every arming
...
CaptureEngine::HeldFrame& CaptureEngine::pushSlot() {
    ...
    if (historyCount_ < kHistorySlots) historyCount_++;
    else historyOldest_ = (historyOldest_ + 1) % kHistorySlots;
...
int CaptureEngine::pollPacket(uint8_t* out, int outCap, uint64_t* frameNumber, uint32_t* flags) {
    *flags = 0;
    ...
    bool debtWaiting = false;
    {
        std::lock_guard<std::mutex> lk(spurtMutex_);
        if (terminatorDebt_ != TerminatorDebt::None) {
            // The claim itself refuses while committed frames are queued — a close (even with a
            // reopen behind it) can land mid-poll, after this poll's reset branch already ran,
            // and a terminator ahead of its spurt's audio inverts time on the wire. Branch order
            // alone cannot guarantee this; the guard has to live here.
            if (readyFrames_ > 0) {
                debtWaiting = true;
            } else {
