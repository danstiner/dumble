...
int PlayoutEngine::slotFor(int32_t session) {
    // The one path that allocates under mutex_, and it does so on the reader thread while a
    // THREAD_PRIORITY_URGENT_AUDIO playback thread may be waiting on that mutex with no priority
    // inheritance to lift this one. Once per talk spurt, not once per session, since
    // kRetireIdleTicks releases the speaker shortly after it stops. If the hold ever bites, the
    // fix is to construct outside the lock and re-check under it, which costs nothing but a rare
    // wasted construction.
    int found = -1;
