...
std::unique_ptr<PlayoutEngine> PlayoutEngine::create(int sampleRate, int maxQuantumSamples,
    ...
    // maxQuantumSamples is a size the per-speaker fifo can be built from.
    if (!SpeakerDecoder::create(sampleRate, maxQuantumSamples)) return nullptr;
...
int PlayoutEngine::slotFor(int32_t session) {
    // The one path that does real work under mutex_: SpeakerDecoder::create mallocs an opus
    // decoder and zero-initialises 67 KB of buffers, measured at 21 us on a host x86_64. Once per
    // talk spurt rather than once per session, since kRetireIdleTicks releases the speaker 100 ms
    // after it stops — so a busy channel pays it a few times a second, against a 10 ms tick, on
    // the reader thread while a THREAD_PRIORITY_URGENT_AUDIO playback thread may be waiting on the
    // mutex with no priority inheritance to lift this one. Not worth removing at that price.
    // Preallocating all kMaxSpeakers would commit ~5 MB resident to serve the handful of speakers
    // a channel actually has, and would only move this cost to startup; if the hold ever does
    // bite, the cheaper fix is to construct outside the lock and re-check under it, which costs
    // nothing but a rare wasted construction.
    int found = -1;
    ...
    auto decoder = SpeakerDecoder::create(sampleRate_, maxQuantumSamples_);
    if (!decoder) return -2;
    const int slot = slots_.claim();
    if (slot < 0) return -1;
    sessions_[slot] = session;
    queues_[slot] = std::make_unique<PacketQueue>();
    decoders_[slot] = std::move(decoder);
    idleTicks_[slot] = 0;
    return slot;
}

int PlayoutEngine::offer(int32_t session, const uint8_t* data, int len, bool terminator) {
    // Refused whole, terminator included, unlike the unpriceable payload PacketQueue::offer still
...
int PlayoutEngine::fillQuantum(int16_t* out, int samples, int32_t* sessions, int32_t* liveSpeakers) {
    if (samples <= 0 || samples > maxQuantumSamples_) {
        // Diagnostic, not silence: the JNI seam validates its count against kMaxPacketSamples,
    ...
    // Snapshotted under the lock, and not just the slot indices: sessions_, queues_ and decoders_
    // are written by slotFor on the reader thread, so reading them during the unlocked decode
    // below would be a race even though the values cannot change while the slot stays claimed.
    struct Live {
        int slot;
        int32_t session;
        PacketQueue* queue;
    ...
    int liveCount = 0;
    {
        std::lock_guard<std::mutex> guard(mutex_);
        slots_.forEach([&](int i) {
            live[liveCount++] = Live{i, sessions_[i], queues_[i].get(), decoders_[i].get()};
        });
    }

    for (int n = 0; n < liveCount; n++) {
        const int i = live[n].slot;
        PacketQueue* queue = live[n].queue;
        SpeakerDecoder* decoder = live[n].decoder;
