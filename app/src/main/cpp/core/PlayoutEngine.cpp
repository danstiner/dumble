...
int PlayoutEngine::offer(int32_t session, const uint8_t* data, int len, uint64_t frameNumber,
    ...
    EstimatorEntry& entry = entryFor(session, arrivalMillis);
    // samples holds libopus's negative error code on the malformed path, and observe() takes a
    // duration: a wider negative would wrap the uint64_t cast inside it.
    const int relative =
        entry.estimator.observe(frameNumber, samples > 0 ? samples : 0, arrivalMillis);
...
PlayoutEngine::EstimatorEntry& PlayoutEngine::entryFor(int32_t session, int64_t arrivalMillis) {
    int free = -1;
    int oldest = 0;
    for (int i = 0; i < kEstimatorSlots; i++) {
        EstimatorEntry& entry = estimators_[i];
        if (!entry.used) {
            if (free < 0) free = i;
        } else if (entry.session == session) {
            entry.lastArrivalMillis = arrivalMillis;
            return entry;
        } else if (estimators_[oldest].used &&
                   entry.lastArrivalMillis < estimators_[oldest].lastArrivalMillis) {
            oldest = i;
        }
    }
    EstimatorEntry& claimed = estimators_[free >= 0 ? free : oldest];
    claimed.estimator.reset();
    claimed.session = session;
    claimed.used = true;
    claimed.lastArrivalMillis = arrivalMillis;
    ...
    // is still a rebase.
    if (downlink_.hasData()) claimed.estimator.seedFrom(downlink_);
    return claimed;
}

JitterEstimator* PlayoutEngine::estimatorOrNull(int32_t session) {
    for (auto& entry : estimators_) {
        if (entry.used && entry.session == session) return &entry.estimator;
    }
    return nullptr;
}
...
int PlayoutEngine::fillQuantum(int16_t* out, int samples, int32_t* sessions,
    ...
    int liveCount = 0;
    {
        ...
        for (int i = 0; i < kMaxSpeakers; i++) {
            ...
            const JitterEstimator* est = estimatorOrNull(sessions_[i]);
    ...
    for (int n = 0; n < liveCount; n++) {
        ...
        while (decoder.available() < samples) {
            ...
            {
                std::lock_guard<std::mutex> guard(mutex_);
                const JitterEstimator* est = estimatorOrNull(sessions_[live[n]]);
    ...
    int producing = 0;
    {
        ...
        for (int n = 0; n < liveCount; n++) {
            ...
            if (queue.gateOpen()) {
                if (JitterEstimator* est = estimatorOrNull(sessions_[i])) est->clearDiscontinuity();
            }
