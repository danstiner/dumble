...
/**
 * Single-producer single-consumer ring buffer specialized for int16 PCM samples. write()
 * allocates nothing, takes no lock, and never blocks, because both producers are on an audio
 * deadline: a SCHED_FIFO callback on the capture side, and the decode step of the playback thread
 * on the playout side.
 *
