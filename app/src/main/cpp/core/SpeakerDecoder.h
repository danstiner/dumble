...
public:
     ...
     *  sender: libopus keeps prediction state between packets, and it must not carry one speaker's
     *  history into another's first packet. Consumer side, like the rest of this class — PcmRing
     *  only lets its consumer move the read index. */
    void reset();
