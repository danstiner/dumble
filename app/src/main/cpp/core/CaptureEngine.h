...
private:
    ...
    // +1: the onset frame transits the queue on its way into the burst. Without it the queue holds
    // 6 slots, the onset frame evicts the oldest, and the burst back-fills 5 frames (50 ms) against
    // the ~62-70 ms onset blind spot. With it, 6 frames (60 ms) covers that blind spot with nothing
    // to spare, not with margin. See docs/capture.md.
    static constexpr int kHistorySlots = kPrerollPackets * kFramesPerPacket + 1;
