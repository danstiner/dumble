...
/**
 * The session as the platform sees it, without a Telecom call: the audio mode, the route and the
 * microphone service. How Discord runs a voice channel (measured on a Pixel 7a, 2026-09-23): its
 * own MODE_IN_COMMUNICATION, SCO opened by AudioService for the communication device it sets, no
 * audio focus. With no Telecom call no InCallService is handed one — so no dialer that takes
 * self-managed calls draws its screen over ours — and Bluetooth still gets SCO, which a Telecom
 * call hidden from those dialers does not.
 *
 * No audio focus is requested, so other apps' media keeps playing, at call quality over a
 * headset's SCO link.
 *
...
class AndroidVoiceCall(private val context: Context) : VoiceCall {
    ...
    private fun handleStart(gen: Int, host: String, onRoutes: (AudioRoutes) -> Unit) {
        ...
        if (!superseding) {
            // Route first: measured, SCO was up a second before the mode landed; mode first
            // (Discord's order) spends ~1 s on the earpiece.
