    ...
    private suspend fun handleStart(
        ...
        val job = scope.launch {
            try {
                ...
                if (!registered) {
                    // Never withhold our calls from InCallServices (the account extra
                    // ADD_SELF_MANAGED_CALLS_TO_INCALLSERVICE): Bluetooth's own is one, and a
                    // headset gets no voice link for a call Bluetooth cannot see.
