...
evaluation (`app/src/test/cpp/EvalCorpus.h`, `app/src/test/cpp/VoiceActivityEvalTest.cpp`).
Ground truth is the hand-drawn `.manual.txt` labels — not an energy heuristic — so when the
gate disagrees with a label, the gate is what's wrong.
