...
TEST(PlayoutEngine, RefusesAQuantumLargerThanTheEngineWasBuiltFor) {
    ...
    // kMaxPacketSamples, the absolute ceiling, and never learns what maxQuantumSamples this engine
...
// The tick, its prebuffer re-arm and its two retirement windows are the engine's, so the cases
// that used to exercise them through SpeakerQueue are engine tests now.
namespace {
...
TEST(PlayoutEngine, SixtyMillisecondSenderDrainsOneQuantumPerTick) {
    auto e = newEngine();
    // The same 60 ms as one packet: a tick must take a fraction of a frame and keep the remainder.
