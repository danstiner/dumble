...
TEST(PacketAssembler, BuildsExactFramesFromAnyDeviceBurstSize) {
    for (uint32_t burst : {96u, 128u, 192u, 240u, 256u}) {
        PcmRing ring(16384);
        PacketAssembler fa(dumble::kTxPacketSamples);
        std::vector<int16_t> frame(dumble::kTxPacketSamples);
        int16_t next = 0;
...
TEST(PacketAssembler, FlushZeroPadsAShortTail) {
    ...
    std::vector<int16_t> frame(dumble::kTxPacketSamples, -1);
    ...
    for (int i = 100; i < dumble::kTxPacketSamples; i++) EXPECT_EQ(0, frame[i]);
}
...
TEST(PacketAssembler, FlushOnAnEmptyRingStillProducesASilentFrame) {
    PcmRing ring(1024);
    PacketAssembler fa(dumble::kTxPacketSamples);
    std::vector<int16_t> frame(dumble::kTxPacketSamples, -1);
    fa.flushFrame(ring, frame.data(), uint32_t(dumble::kTxPacketSamples));
    for (int i = 0; i < dumble::kTxPacketSamples; i++) EXPECT_EQ(0, frame[i]);
}
