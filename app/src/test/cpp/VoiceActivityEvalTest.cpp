 ...
 * modelled: the engine holds kPrerollFrames frames and flushes them when the gate opens, so
...
Trace transmitTrace(const Clip& clip, VoiceActivity& va) {
    constexpr int kPrerollFrames = dumble::kPrerollFrames;
...
TEST(VoiceActivityEval, MeetsThePinnedBars) {
    ...
    std::printf("[          ] open=%.2f close=%.2f hangover=%d frames preroll=%d frames\n",
                dumble::kOpenLevel, dumble::kCloseLevel, dumble::kHangoverFrames,
                dumble::kPrerollFrames);
...
std::vector<bool> engineTransmitTrace(const Clip& clip) {
    ...
    for (int f = 0; f < frames; f++) {
        ...
        // Drain: an opening edge emits its burst in one poll, so one poll per frame would fall
        // behind until the burst finishes.
