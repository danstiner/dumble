...
class FakePlayoutEngine : VoiceReceiver.PlayoutEngine {
    ...
    /** Like the adapter's pause: the engine's speakers are released with the stream. */
    override fun pause() {
        checkLive()
        running = false
        liveSessions = emptySet()
        audibleSessions = emptySet()
        calls += "pause"
    }
    override fun destroy() { calls += "destroy" }

    /** A call after destroy() is the use-after-free the receiver's join exists to prevent. */
    private fun checkLive() = check(!destroyed) { "engine used after destroy()" }
}
